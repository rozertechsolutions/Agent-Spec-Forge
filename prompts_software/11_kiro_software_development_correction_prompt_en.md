# AI Specialized Departments — Kiro Software Development Correction Prompt
...
Correct the existing **Software Development** implementation for **Kiro** in place.

**Previous audit result:** `PASS WITH MAJOR CORRECTIONS`  
**Selected product surface:** Kiro IDE steering, Skills, and current IDE custom-agent Markdown files; no mixing with Kiro CLI JSON agents
...
1. Treat this package as a Kiro IDE implementation. State that product surface explicitly in `README.md`.
2. Keep `.kiro/steering/` authoritative for the primary Kiro agent, which acts as Software Development Lead.
3. Add seven Kiro IDE specialist custom agents using the exact current native Markdown location and schema. Do not create a Lead custom agent.
4. Specialists: requirements/planning, architecture, implementation/maintenance, test/quality, code-quality review, engineering-risk review, and documentation/release readiness.
5. Keep reviewers/planners read-only. Give the implementation specialist only approval-gated workspace editing. No shell, network, MCP, hooks, deployment, publication, signing, release, credentials, or Git mutation.
6. Every specialist must report to the primary Kiro agent and must not recursively delegate or claim final completion.
7. Do not create Kiro CLI JSON agent files in this IDE package. Delete any mixed-surface JSON agents if present.
8. Preserve `.kiro/skills/` and remove scripts or duplicated policy content.
9. Keep `.kiro/specs/` absent because this correction is not a concrete feature specification.
10. Keep hooks and MCP absent.
11. Deepen all eleven workflow references. If Kiro IDE has no repository-native workflow loader for their current location, keep them clearly auxiliary and do not claim otherwise.

## Strictly justified deletion policy for this platform

- Delete any custom Lead agent.
- Delete Kiro CLI JSON agents or configuration mixed into the Kiro IDE package.
- Delete generic `.kiro/specs/`, hooks, MCP configuration, scripts, or invented schemas.
- Delete duplicate workflow references after preserving unique content.

## Required corrected target state

A Kiro IDE package with steering-led orchestration, seven native IDE specialists, Skills, differentiated workflow references, and no CLI/IDE format mixing.
...
1. Read and inventory only the existing files beneath `kiro/software-development/` using direct file APIs.
