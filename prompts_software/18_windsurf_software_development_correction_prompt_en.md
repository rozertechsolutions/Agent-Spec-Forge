# AI Specialized Departments — Windsurf Software Development Correction Prompt
...
Correct the existing **Software Development** implementation for **Windsurf** in place.

**Previous audit result:** `PASS`  
**Selected product surface:** `AGENTS.md`, `.windsurf/rules/`, `.windsurf/skills/`, and `.windsurf/workflows/` for Cascade
...
1. Preserve the current native structure and make the primary Cascade session the Software Development Lead through `AGENTS.md`.
2. Do not create custom-agent files if Windsurf has no native project custom-agent format for them.
3. Consolidate duplicated global policy text between `AGENTS.md`, rules, Skills, workflows, and `README.md`.
4. Rewrite all eleven `.windsurf/workflows/*.md` files so each has distinct professional gates and evidence.
5. Check rule activation metadata and workflow frontmatter against the current native format already used in the target. Remove invented or unnecessary metadata.
6. Keep Skills complete, reusable, non-executable, and narrowly scoped.
7. Ensure every workflow stops for human approval before editing sensitive scope, changing architecture/dependencies, running commands, accessing external systems, deploying, publishing, signing, releasing, or performing destructive operations.
8. Keep hooks, MCP, shell helpers, auto-run configuration, background jobs, and external integrations absent.
9. A no-change result is valid for files already correct; do not rewrite them cosmetically.

## Strictly justified deletion policy for this platform

- Delete `.windsurf/hooks.json`, MCP configuration, executable helpers, background automation, fake custom agents, or unsupported metadata if present.
- Delete duplicate workflow/reference files after preserving unique content.

## Required corrected target state

A native rules/Skills/workflows package led by the primary Cascade session, with differentiated workflows, minimal duplication, and no hooks, MCP, or simulated agents.
...
1. Read and inventory only the existing files beneath `windsurf/software-development/` using direct file APIs.
