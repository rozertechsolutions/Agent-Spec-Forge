# AI Specialized Departments — Warp Software Development Correction Prompt
...
Correct the existing **Software Development** implementation for **Warp** in place.

**Previous audit result:** `PASS`  
**Selected product surface:** project guidance through `WARP.md`/supported rules plus Warp Drive rules, Skills, and workflows where manually imported or natively supported
...
1. Keep the primary Warp Agent as Software Development Lead through `WARP.md`.
2. Verify every claim about automatic repository loading. Clearly separate project guidance that Warp reads natively from Warp Drive material that requires manual import or account/product support.
3. Preserve `warp-drive/rules/`, `warp-drive/skills/`, and `warp-drive/workflows/` only as accurately labelled source material.
4. Do not create repository custom-agent files unless Warp officially supports that exact project format; do not simulate specialists.
5. Express specialist responsibilities as routing stages and review gates within rules, Skills, and workflows.
6. Deepen all eleven workflows and remove generic repetition.
7. Keep cloud agents, Oz/background schedules, terminal launch automation, MCP, connectors, and external integrations disabled and absent.
8. Require interactive human approval before any terminal command, edit, Git action, external action, deployment, publication, signing, or release.
9. Make no change solely to increase native-looking structure. A no-change result is valid if the package already meets every requirement.

## Strictly justified deletion policy for this platform

- Delete files that falsely claim automatic loading.
- Delete cloud-agent schedules, background jobs, MCP configuration, terminal scripts, launchers, auto-approval settings, or fake repository agents.
- Delete duplicate manual-import files after preserving unique content.

## Required corrected target state

An honest Warp package with the primary Agent as Lead, accurate native/manual-import labels, differentiated rules/Skills/workflows, and no cloud/background/terminal automation.
...
1. Read and inventory only the existing files beneath `warp/software-development/` using direct file APIs.
