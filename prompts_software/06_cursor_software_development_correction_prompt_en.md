# AI Specialized Departments — Cursor Software Development Correction Prompt
...
Correct the existing **Software Development** implementation for **Cursor** in place.

**Previous audit result:** `PASS WITH CORRECTIONS`  
**Selected product surface:** `AGENTS.md`, `.cursor/rules/`, `.cursor/skills/`, and Cursor's current native subagent capability where project-defined subagents are supported
...
1. Keep `AGENTS.md` as the authoritative Software Development Lead instruction layer for the primary Cursor Agent.
2. Preserve project rules and Skills, remove duplicated policy prose, and make routing/review separation explicit.
3. Use Cursor's current native subagent mechanism only if the target can be represented with the exact current documented project path and frontmatter. Do not guess a schema.
4. When native project subagents are representable, create only seven specialists and never a Lead subagent. Reviewers must be read-only; the implementation specialist must require human-approved edits and have no shell/network/Git/release/deployment authority.
5. When the exact stable project subagent format cannot be established from the target and this prompt, omit subagent files and document that omission truthfully in `README.md`; do not simulate them.
6. The primary Cursor Agent must retain coordination, scope control, and final evidence aggregation.
7. Deepen all eleven workflow documents. Keep `docs/workflows/` or equivalent explicitly auxiliary unless Cursor natively loads the exact location.
8. Do not create legacy `.cursorrules`, background-agent configuration, hooks, MCP, shell helpers, or automatic approvals.

## Strictly justified deletion policy for this platform

- Delete any `software-development-lead` subagent if present.
- Delete invented or obsolete custom-agent files whose path/frontmatter is not recognized by the current stable product.
- Delete `.cursorrules`, hooks, MCP configuration, background-agent automation, and executable helpers.

## Required corrected target state

The primary Cursor Agent is Lead. Rules and Skills are always valid native components. Specialist subagents exist only when encoded in the exact current native format; otherwise the package remains honest and uses rules/Skills without simulation.
...
1. Read and inventory only the existing files beneath `cursor/software-development/` using direct file APIs.
