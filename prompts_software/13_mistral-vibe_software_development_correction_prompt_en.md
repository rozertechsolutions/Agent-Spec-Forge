# AI Specialized Departments — Mistral Vibe Software Development Correction Prompt
...
Correct the existing **Software Development** implementation for **Mistral Vibe** in place.

**Previous audit result:** `FAIL — BLOCKER`  
**Selected product surface:** `AGENTS.md`, `.vibe/config.toml`, custom user agents/subagents in `.vibe/agents/`, prompts, Skills, and Vibe tools
...
1. Keep `software-development-lead` as the user-facing default agent because Vibe supports an `agent_type = "agent"` coordinator.
2. Add the native `task` tool to the Lead's `enabled_tools` so it can delegate to subagents. Do not add write, Bash, web, or external tools to the Lead.
3. Ensure all seven specialists use `agent_type = "subagent"` and return evidence to the Lead.
4. Add only the native editing tools required by the current Vibe schema to the implementation specialist—`write_file` and `search_replace` where those are the documented names—alongside read/search tools.
5. Keep planning, architecture, testing, code-quality, engineering-risk, and documentation/release specialists read-only.
6. Remove all delegation instructions from specialists; only the Lead may use `task`.
7. Treat `safety = "safe"` as metadata, not enforcement. Enforce safety through exact tool allowlists and instructions.
8. Update `.vibe/config.toml` and `README.md` to require explicit selection of the safe Lead for programmatic use. Warn that programmatic/default invocation may otherwise use auto-approval behavior. Do not add a command, script, or wrapper to enforce this.
9. Do not pin a model, provider, endpoint, credential, or account.
10. Keep Bash absent from all agents. Keep MCP, hooks, network, deployment, publication, signing, release, and Git mutation absent.
11. Deepen the eleven workflow documents and update role prompts so only the Lead aggregates completion.

## Strictly justified deletion policy for this platform

- Delete any specialist delegation capability or `task` permission.
- Delete any Bash, web, MCP, deployment, publication, signing, release, or Git tool grants.
- Delete model/provider/endpoint pins, scripts, hooks, or automatic launch helpers.
- Delete duplicated workflow documents only after complete replacement.

## Required corrected target state

A Vibe Lead agent with read/search plus `task`; seven subagents; only the implementation subagent can edit through minimum native tools. Safe interactive/programmatic selection limitations are documented explicitly.
...
1. Read and inventory only the existing files beneath `mistral-vibe/software-development/` using direct file APIs.
