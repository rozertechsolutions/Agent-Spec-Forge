# AI Specialized Departments — ChatGPT Software Development Correction Prompt
...
Correct the existing **Software Development** implementation for **ChatGPT** in place.

**Previous audit result:** `PASS WITH LIMITATIONS`  
**Selected product surface:** manual ChatGPT Projects, Custom GPTs, Skills when available, Workspace Agents when available, and manually configured apps/connectors
...
1. Preserve the package as a manual source package. No repository file may claim that ChatGPT automatically loads it.
2. Review `README.md`, `project/`, `custom-gpt/`, `skills/`, `workspace-agent/`, and `apps-connectors/` for accurate manual-import or copy/paste instructions.
3. Clearly separate generally available Project content from account-, plan-, admin-, or workspace-dependent Custom GPT, Skill, Workspace Agent, app, and connector features.
4. Consolidate repeated operating-model, safety, and quality text. Keep one authoritative definition and use precise internal references instead of duplicated policy blocks.
5. Ensure the eleven professional workflows are meaningfully differentiated inside native instructions or knowledge documents. Do not create a fake repository workflow engine.
6. Keep every connector/app disabled and unconfigured. Remove any real identifiers, endpoints, credentials, authentication steps, or language implying automatic activation.
7. Keep Skills as static source packages only where the package accurately describes them as conditional on current Skills support. Remove scripts, executable assets, or unsupported manifest files if present.
8. Keep `workspace-agent/` only as a clearly conditional manual specification. Delete it if it claims repository auto-loading or unconditional availability and cannot be corrected truthfully.
9. Do not create `.chatgpt/`, repository agents, hooks, MCP configuration, permission manifests, installers, or automation files.

## Strictly justified deletion policy for this platform

- Delete duplicate or contradictory knowledge documents only after their unique content has been merged into the authoritative document.
- Delete any file that presents a manual UI specification as a native repository-loaded configuration.
- Delete unsupported Skill manifests, scripts, executable assets, or connector configuration containing activation data.

## Required corrected target state

A compact, honest manual package for Projects and other supported ChatGPT surfaces. Project instructions own orchestration; specialist responsibilities are expressed through instructions, Skills, or knowledge—not fake repository subagents. All conditional surfaces are labelled with their plan/workspace limitation.
...
1. Read and inventory only the existing files beneath `chatgpt/software-development/` using direct file APIs.
