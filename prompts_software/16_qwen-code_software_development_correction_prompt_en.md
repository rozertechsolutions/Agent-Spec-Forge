# AI Specialized Departments — Qwen Code Software Development Correction Prompt
...
Correct the existing **Software Development** implementation for **Qwen Code** in place.

**Previous audit result:** `FAIL — BLOCKER`  
**Selected product surface:** `QWEN.md`, `.qwen/settings.json`, `.qwen/agents/*.md`, `.qwen/skills/`, and current Markdown commands
...
1. Delete `.qwen/agents/software-development-lead.md`. The main Qwen Code session must be the Software Development Lead.
2. Update `QWEN.md` so the main session owns scope, planning, delegation, approvals, review ordering, evidence aggregation, and final reporting.
3. Create `.qwen/settings.json` containing:
   - `"tools": { "approvalMode": "default" }`;
   - no model providers, selected model, authentication data, environment values, MCP, hooks, extensions, endpoints, credentials, or sandbox builders.
4. Retain seven specialists and set explicit `approvalMode` in every agent:
   - requirements/planning, architecture, testing, code-quality review, engineering-risk review, and documentation/release readiness: `approvalMode: plan`;
   - implementation/maintenance: `approvalMode: default`.
5. Keep exact tool allowlists:
   - read-only roles: read/search/list tools only;
   - implementation: read/search plus `write_file` and `replace`;
   - all roles must disallow `run_shell_command` and omit MCP/browser/network tools.
6. State in `QWEN.md` and `README.md` that a permissive parent session such as `auto-edit` or `yolo` can override subagent restrictions. Require users to start and keep the parent in `default` mode; do not claim project settings can defeat explicit permissive CLI flags.
7. Remove every specialist instruction to delegate. Qwen specialists return to the main session and never call each other.
8. Review all `.qwen/commands/*.md` files: use the current Markdown command format, prompt-only content, main-session coordination, and no deprecated TOML commands.
9. Deepen the eleven workflows.
10. Keep Skills non-executable and remove duplicated policy prose.

## Strictly justified deletion policy for this platform

- Delete `.qwen/agents/software-development-lead.md`.
- Delete deprecated TOML commands, hooks, MCP configuration, model/provider/auth configuration, scripts, and YOLO/auto-edit defaults.
- Delete duplicated workflow or policy files after preserving unique content.

## Required corrected target state

Qwen's main session is Lead; seven specialists use explicit approval modes; project default is Ask Permissions; only the implementation specialist can request file edits; no shell or external tools exist.
...
1. Read and inventory only the existing files beneath `qwen-code/software-development/` using direct file APIs.
