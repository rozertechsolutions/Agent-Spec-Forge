...
Product surface: Cursor Project Rules, AGENTS.md-compatible guidance, Skills, subagents, hooks, MCP, and IDE agent permissions.
...
Cursor IDE with trusted workspace; repository rules enabled; no integrations enabled by this package.
...
Open the repository or an area folder in Cursor. Keep .cursor rules and Skills under each area; enable Skills/subagents only where the current IDE supports them.
...
Cursor discovers project rules from documented .cursor/rules locations and relevant workspace instructions. Area isolation is by area-local rule files and selected working directory.
...
- `governance-risk-compliance-assurance/`: `AGENTS.md`, `.cursor/`
- `security-architecture-engineering/`: `AGENTS.md`, `.cursor/`
- `application-product-devsecops-security/`: `AGENTS.md`, `.cursor/`
- `exposure-vulnerability-hardening/`: `AGENTS.md`, `.cursor/`
- `defensive-security-operations-detection-intelligence/`: `AGENTS.md`, `.cursor/`
- `incident-response-dfir-recovery/`: `AGENTS.md`, `.cursor/`
- `offensive-security-authorized-validation/`: `AGENTS.md`, `.cursor/`
- `cyber-resilience-specialized-technologies/`: `AGENTS.md`, `.cursor/`
...
Default behavior is read-only and static. Repository writes, where a platform technically allows them, must stay inside the selected `cursor/cybersecurity/<area>/` directory and require an explicit user task to update static artifacts. Shell, network, installation, deployment, scanning, exploitation, recovery execution, remote Git operations, MCP connections, hosted tools, and external connectors are prohibited by default.
