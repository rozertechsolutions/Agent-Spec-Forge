---
description: "user-invoked Incident Response, DFIR, and Recovery workflow for `evidence-forensics-planning`. This command is a prompt artifact only and is not executed during repository implementation."
---

# /evidence-forensics-planning

Purpose: user-invoked Incident Response, DFIR, and Recovery workflow for `evidence-forensics-planning`. This command is a prompt artifact only and is not executed during repository implementation.

Authorized scope: static repository files and supplied redacted evidence for `incident-response-dfir-recovery`.
...
2. Invoke the relevant Skill from: incident-readiness-triage, evidence-forensics-planning, containment-recovery-coordination, scenario-tabletop-post-incident, independent-incident-recovery-assurance.
3. Assign one primary owner from: incident-command-evidence-agent, forensics-containment-recovery-agent, scenario-crisis-review-agent, post-incident-corrective-action-agent, independent-incident-recovery-reviewer.
