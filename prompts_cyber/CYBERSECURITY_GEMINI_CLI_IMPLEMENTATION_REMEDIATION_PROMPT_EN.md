...
## Platform Implementation and Remediation Prompt: Gemini CLI
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Gemini CLI**.
...
- GEMINI.md or the current persistent instruction mechanism
- Agent Skills
- Subagents
- Custom commands and prompts
- Hooks
- MCP
- Extensions that package supported components
...
- Create the complete `gemini-cli/cybersecurity/` tree from scratch.
- Choose the current official project-scoped surface: native project files or an extension package. Do not mix both without a documented reason.
- Use Skills, subagents, and commands for the eight professional areas.
- Use hooks only as reviewed safety gates and never execute them during implementation.
- Keep MCP absent or disabled and unconnected.

## Explicit platform exclusions and corrections

- Do not copy old Gemini CLI formats or issue-proposal formats.
- Do not configure YOLO or broad auto-approval.
- Do not create an extension manifest containing components that the current extension runtime does not support.
- Do not execute extension refresh, hooks, commands, or tools.

## Platform-specific audit requirements

- This platform was absent from the audited ZIP and must receive all eight areas.
- Verify the July 2026 extension, Skill, subagent, hook, command, and MCP formats from the official repository documentation.
- Document folder trust and approval limitations.
...
A platform-level coverage or native-capability record may be created under `gemini-cli/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
feat(gemini-cli): add complete cybersecurity specialization
```
