...
## Platform Implementation and Remediation Prompt: Claude web/desktop
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Claude web/desktop**.
...
- Project knowledge
- Claude Skills
- Claude connectors configured through the supported UI
...
- Build an importable/manual Claude Project package for each area.
- Use project instructions for persistent area authority, knowledge files for reference material and schemas, and Skills for reusable workflows.
- Document connector options without activating, authenticating, or embedding real connector configuration.

## Explicit platform exclusions and corrections

- Do not create Claude Code files, `.claude/agents`, Claude Code hooks, terminal permissions, or repository MCP configuration.
- Do not simulate subagents or automated workflows that Claude Projects do not natively expose.
- Do not claim connectors are available or enabled on every plan.

## Platform-specific audit requirements

- Audit the existing package for accidental Claude Code concepts.
- Ensure every area can be used as a self-contained Claude Project with exact instructions, knowledge inventory, Skills, setup steps, and plan limitations.
- Keep connectors optional and manual.
...
A platform-level coverage or native-capability record may be created under `claude/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(claude): harden complete cybersecurity specialization
```
