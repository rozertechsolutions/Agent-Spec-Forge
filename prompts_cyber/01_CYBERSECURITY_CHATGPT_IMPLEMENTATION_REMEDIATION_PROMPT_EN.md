...
## Platform Implementation and Remediation Prompt: ChatGPT
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **ChatGPT**.
...
- ChatGPT Projects with project instructions and uploaded project knowledge
- ChatGPT Skills
- Custom GPT configuration through the GPT builder
- Workspace Agents where the user's Business, Enterprise, or Edu workspace supports them
- Apps and connectors configured manually in the ChatGPT UI
...
- OpenAI ChatGPT Business / Enterprise / Edu release notes — Workspace Agents
- OpenAI Help Center — ChatGPT agent and apps/connectors
...
- Treat this as an import and manual-configuration package, not as a repository-executed coding-agent configuration.
- For each area, create precise Project instructions, a curated project-knowledge map, reusable ChatGPT Skills, and copy-ready Custom GPT or Workspace Agent builder specifications only where that surface provides real value.
- Document plan and workspace limitations. Do not claim Workspace Agents are available to every account.
- Keep all apps and connectors optional, manually configured, and disabled or unconnected by default.

## Explicit platform exclusions and corrections

- Do not create fake repository agents or subagents.
- Do not create hooks, MCP configuration, terminal permissions, or executable workflows.
- Do not represent manual GPT or Workspace Agent settings as an official repository schema.
- Do not create a folder merely named `agents/` to imitate coding-agent platforms.

## Platform-specific audit requirements

- Review the existing eight-area package and remove any artifact that suggests ChatGPT automatically loads repository files.
- Ensure every manual configuration file states the exact UI surface, eligible plans, import steps, human approval points, and limitations.
- Use ChatGPT Skills for reusable procedures; use Projects for area context; use Workspace Agents only as optional workspace-level configurations.
...
A platform-level coverage or native-capability record may be created under `chatgpt/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(chatgpt): harden complete cybersecurity specialization
```
