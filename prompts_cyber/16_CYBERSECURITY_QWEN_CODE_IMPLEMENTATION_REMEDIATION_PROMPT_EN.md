...
## Platform Implementation and Remediation Prompt: Qwen Code
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Qwen Code**.
...
- Qwen Code Docs — Agent Skills
- Qwen Code Docs — Subagents
- Qwen Code Docs — Commands
- Qwen Code Docs — Hooks
- Qwen Code Docs — MCP
- Qwen Code Docs — Extensions
...
- Select one current official repository packaging model and use it consistently.
- Use complete Skills, focused subagents, and explicit commands.
- Use hooks only as static, reviewed safety gates and never execute them during implementation.
- Keep MCP absent or disabled.
- Add the missing defensive-operations area.

## Explicit platform exclusions and corrections

- Do not mix obsolete extension formats with the July 2026 runtime.
- Do not enable YOLO or broad auto-approval.
- Do not create model, provider, authentication, or live MCP configuration.
- Do not rely on blog announcements instead of current official reference pages.

## Platform-specific audit requirements

- The audited package lacks area 05; create it.
- Verify exact current extension refresh and discovery behavior.
- Ensure Skills use the current invocation model and valid frontmatter.
...
A platform-level coverage or native-capability record may be created under `qwen-code/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(qwen-code): complete and harden cybersecurity specialization
```
