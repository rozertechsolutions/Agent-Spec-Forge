...
## Platform Implementation and Remediation Prompt: Warp / Oz
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Warp / Oz**.
...
- Project Rules through AGENTS.md or WARP.md
- Warp Skills
- Skills as agent instructions
- Local and Oz agent configuration concepts
- MCP for local or cloud agents when explicitly configured
...
- Use version-controlled Project Rules and Skills as the primary repository artifacts.
- Use Skills as specialized agent procedures where Warp's current surface supports them.
- Do not invent repository custom-agent files if the current official format is UI/API-only.
- Document local-agent versus Oz-cloud-agent limitations.
- Add the missing defensive-operations area.

## Explicit platform exclusions and corrections

- Do not create cloud environments, schedules, API tasks, remote branches, or autonomous Oz runs.
- Do not connect MCP or specify real environments.
- Do not create a fake workflows folder when no current repository workflow format exists.
- Do not make cloud-agent availability a universal assumption.

## Platform-specific audit requirements

- The audited package lacks area 05; create it.
- Review whether existing files are truly discovered by current Warp.
- Prefer portable Skills and Project Rules over descriptive files that Warp does not load.
...
A platform-level coverage or native-capability record may be created under `warp/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(warp): complete and harden cybersecurity specialization
```
