...
## Platform Implementation and Remediation Prompt: Kiro
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Kiro**.
...
- Steering and AGENTS.md
- Agent Skills
- Workspace custom agents and subagents
- Specs
- Agent hooks
- MCP and tool restrictions
...
- Use steering for persistent area standards and Skills for reusable professional workflows.
- Use custom agents only for responsibilities needing isolated context and explicit tool restrictions.
- Use Specs for structured requirements/design/task workflows only where that development-oriented surface improves the Cybersecurity area.
- Use hooks only when a native prompt or reviewed guardrail provides real value; never run them during implementation.
- Keep MCP absent or disabled.

## Explicit platform exclusions and corrections

- Do not give subagents Specs access when the official surface does not support it.
- Do not assume hooks trigger in subagents.
- Do not create shell-command hooks merely to fill the hooks component.
- Do not access or modify global steering, agents, or settings.

## Platform-specific audit requirements

- Expand the short Kiro agents identified by the audit.
- Remove unsafe or simulated hooks.
- Ensure steering inclusion modes are selected intentionally and do not load all eight areas into every task unnecessarily.
...
A platform-level coverage or native-capability record may be created under `kiro/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(kiro): harden complete cybersecurity specialization
```
