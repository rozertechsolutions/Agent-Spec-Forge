...
## Platform Implementation and Remediation Prompt: Cline
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Cline**.
...
- Create the complete `cline/cybersecurity/` tree from scratch.
- Use current project-scoped Cline rules for persistent safety and authority boundaries.
- Use Skills for reusable professional procedures, Workflows for user-invoked multi-stage tasks, and Subagents only for focused ownership and independent review.
- Use Hooks only as native guardrails with reviewable, fail-closed behavior; do not run them during implementation.
- Omit MCP unless a meaningful disabled-by-default optional contract is justified.

## Explicit platform exclusions and corrections

- Do not copy the Claude Code, Cursor, or Kilo directory layout.
- Do not rely on global `~/.cline/` files.
- Do not enable auto-approval or external integrations.
- Do not create active cron, plugin, or MCP behavior.

## Platform-specific audit requirements

- This platform was absent from the audited ZIP and must receive all eight areas.
- Verify exact current project paths and schemas before creation.
- Ensure Cline's action approval model remains conservative and every potentially active action requires human approval.
...
A platform-level coverage or native-capability record may be created under `cline/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
feat(cline): add complete cybersecurity specialization
```
