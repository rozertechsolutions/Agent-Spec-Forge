...
## Platform Implementation and Remediation Prompt: Codex
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Codex**.
...
- Agent Skills
- Hooks
- Commands where currently supported
- Project `.codex/config.toml` and permissions
- MCP configuration
...
- OpenAI Developers — Codex AGENTS.md
...
- Correct the entire existing Codex implementation before adding content.
- Every `.codex/agents/*.toml` file must use the exact current schema and include `name`, `description`, and `developer_instructions` where required.
- Move unsupported concepts such as `allowed_outputs` into explicit natural-language instructions or supported fields.
- Use valid direct `approval_policy` syntax or the documented granular form.
- Keep all project permissions minimal and valid.
- Use AGENTS.md for durable area-level rules, Skills for reusable workflows, subagents for exclusive specialist responsibilities, and hooks only through the current official schema.

## Explicit platform exclusions and corrections

- Remove or replace unsupported custom keys including `static_only`, `external_integrations_enabled`, `production_actions_enabled`, `live_security_actions_enabled`, unsupported `[authority]`, and unsupported `[handoff]` tables.
- Do not use `instructions` in place of required `developer_instructions`.
- Do not use project-local `profile` as a selector when the official documentation disallows it.
- Do not create fake MCP servers or unsupported agent fields.
- Do not allow shell, network, installation, build, test, deployment, scanning, or destructive Git operations.

## Platform-specific audit requirements

- The audit found 18 invalid agent configurations. Review all existing Codex agents, not only those already identified.
- Review all eight `.codex/config.toml` files against the current official schema.
- Ensure the defensive-operations area has complete agents, Skills, workflows, and independent review.
- Record every removed invalid key and every corrected file in the final report.
...
A platform-level coverage or native-capability record may be created under `codex/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(codex): correct and harden complete cybersecurity specialization
```
