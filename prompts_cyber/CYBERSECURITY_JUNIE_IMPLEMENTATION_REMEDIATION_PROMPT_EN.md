...
## Platform Implementation and Remediation Prompt: JetBrains Junie
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **JetBrains Junie**.
...
- `.junie/AGENTS.md` guidelines
- `.junie/skills/` Agent Skills
- Project custom subagents
- `.junie/commands/` slash commands
- MCP configuration and locations
- Action approval and allowlist behavior
...
- Use `.junie/AGENTS.md` as the current preferred project guideline file.
- Migrate or remove legacy `.junie/guidelines.md` content unless an official compatibility reason requires it.
- Use native Skills, project subagents, and custom commands.
- Use read-only tool restrictions for reviewer and assurance subagents.
- Keep MCP unconfigured and unconnected by default.

## Explicit platform exclusions and corrections

- Do not rely on project-level hooks: current official documentation states hooks from the default project configuration are ignored for safety.
- Do not enable Brave mode or add broad Action Allowlist entries.
- Do not add secrets, provider keys, proxy endpoints, or global configuration.
- Do not create nominal one-sentence agents or commands.

## Platform-specific audit requirements

- The audit found many short Junie agents and commands. Expand the underlying agent or Skill contract rather than padding command text.
- Use commands as thin, explicit entry points only when they delegate to complete Skills or subagents.
- Record that project hooks were intentionally omitted because they are not a valid repository safety mechanism.
...
A platform-level coverage or native-capability record may be created under `junie/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(junie): harden complete cybersecurity specialization
```
