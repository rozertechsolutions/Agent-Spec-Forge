...
## Platform Implementation and Remediation Prompt: Mistral Vibe Code
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Mistral Vibe Code**.
...
- Repository `.vibe/` configuration
- Custom agents
- Agent Skills
- Tool filters and per-tool permissions
- MCP configuration
- Slash-command exposure through Skills where supported
...
- Use project `.vibe/` configuration only after verifying the current exact paths and schemas.
- Create read-only or tightly scoped agents, complete Skills, and valid tool restrictions.
- Disable bash, MCP tools, connectors, external access, and unnecessary file edits.
- Add the missing defensive-operations area.
- Document trusted-folder behavior and programmatic-mode approval limitations.

## Explicit platform exclusions and corrections

- Do not select `auto-approve`.
- Do not rely on the programmatic default that can fall back to auto-approval.
- Do not add provider keys, active models, real endpoints, or MCP servers.
- Do not enable telemetry or auto-update settings as part of this specialization unless the current project already owns those settings.

## Platform-specific audit requirements

- The audited package lacks area 05; create it.
- Review every agent's tool set and approval rules.
- Ensure the package does not overwrite unrelated root `.vibe/config.toml` settings; use merge-safe area documentation or scoped files where native.
...
```text
mistral-vibe/cybersecurity/<area>/
```
...
A platform-level coverage or native-capability record may be created under `mistral-vibe/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(mistral-vibe): complete and harden cybersecurity specialization
```
