...
## Platform Implementation and Remediation Prompt: Cursor
...
```text
cursor/cybersecurity/
```

The platform directory was missing from the audited package. Create it from scratch.
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Cursor**.
...
- Hooks
- MCP
- Agent and CLI permission behavior
...
- Create the complete `cursor/cybersecurity/` tree from scratch.
- Use `.cursor/rules/` or the current documented repository-scoped rules format for persistent boundaries.
- Use current Cursor Agent Skills and custom subagents for focused specialization and independent review.
- Use hooks only for safe, explicit guardrails and never execute them during implementation.
- Omit MCP unless a real optional integration is justified and disabled by default.

## Explicit platform exclusions and corrections

- Do not use legacy `.cursorrules` when current documentation requires project rules.
- Do not copy another platform's agent schema.
- Do not enable cloud agents, remote actions, terminal execution, or MCP connections by default.
- Do not create scripts inside Skills unless they are genuinely necessary, fully documented, and remain unexecuted.

## Platform-specific audit requirements

- This platform was absent from the audited ZIP and must receive all eight areas.
- Verify current paths and frontmatter for subagents, Skills, Rules, and Hooks.
- Use read-only and planning-oriented permissions for cybersecurity analysis; file edits must remain limited to generated artifacts.
...
A platform-level coverage or native-capability record may be created under `cursor/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
feat(cursor): add complete cybersecurity specialization
```
