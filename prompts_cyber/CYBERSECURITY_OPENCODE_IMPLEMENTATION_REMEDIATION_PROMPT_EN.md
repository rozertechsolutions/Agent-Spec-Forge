...
## Platform Implementation and Remediation Prompt: OpenCode
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **OpenCode**.
...
- Use AGENTS.md for durable safety rules, native agents/subagents for exclusive responsibility, Skills for reusable methods, and commands for explicit workflows.
- Use granular permission rules with deny-by-default shell, external, and write behavior.
- Add the missing defensive-operations area.
- Remove simulated MCP examples.

## Explicit platform exclusions and corrections

- Remove every `example.invalid` or dummy MCP configuration.
- Do not configure local or remote MCP servers without a real approved integration.
- Do not allow bash, network access, or unrelated edits.
- Do not create duplicate primary agents and subagents with the same authority.

## Platform-specific audit requirements

- The audited package lacks area 05; create it.
- Review exact `.opencode` paths and `opencode.json` schema.
- Ensure commands reference complete Skills or agents and specify `subtask` behavior deliberately.
...
A platform-level coverage or native-capability record may be created under `opencode/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(opencode): complete and harden cybersecurity specialization
```
