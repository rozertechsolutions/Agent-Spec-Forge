...
## Platform Implementation and Remediation Prompt: GitHub Copilot
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **GitHub Copilot**.
...
- Agent Skills
- Prompt files
- Hooks for supported Copilot agent surfaces
- MCP in supported agent configurations
...
- Audit and harden all eight existing areas.
- Use repository or path-specific instructions for durable safety and context.
- Use custom agents for focused roles, Skills for reusable methods, and prompt files for explicitly invoked workflows.
- Use hooks only on the exact supported Copilot surface and only as reviewed safety gates.
- Keep MCP absent or disabled unless a real integration is documented.

## Explicit platform exclusions and corrections

- Do not assume every IDE or Copilot surface supports the same custom-agent or hook features.
- Do not create legacy instruction formats.
- Do not grant broad tools or shell access to cybersecurity agents.
- Do not duplicate a full workflow in an agent, Skill, and prompt file without an explicit delegation model.

## Platform-specific audit requirements

- Expand brief custom agents, Skills, and prompt files found by the audit.
- Document public-preview limitations where relevant.
- Ensure each custom agent's tools and MCP definitions follow the exact official frontmatter schema.
...
```text
github-copilot/cybersecurity/<area>/
```
...
A platform-level coverage or native-capability record may be created under `github-copilot/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(github-copilot): harden complete cybersecurity specialization
```
