...
## Platform Implementation and Remediation Prompt: Kilo Code
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Kilo Code**.
...
- Current agent Markdown files with YAML frontmatter
- Primary agents and subagents
- Per-agent permissions
- `.kilo/skills/` Agent Skills
- Workflows
- Custom Rules and AGENTS.md
- `kilo.jsonc` where current configuration requires it
...
- Migrate all relevant content to the current Kilo CLI-backed format.
- Use `.kilo/agents/*.md` or another currently documented canonical path, valid YAML frontmatter, `mode`, and explicit per-agent permissions.
- Use `.kilo/skills/<skill>/SKILL.md` with correct name/description frontmatter.
- Use current Workflows and Rules only when they add distinct native value.
- Use permission rules to deny bash and restrict edits.

## Explicit platform exclusions and corrections

- Do not use legacy `custom_modes.yaml` or `.kilocodemodes` unless official documentation proves the selected installed surface still requires them.
- Do not retain obsolete `.kilo/agent` or `.kilocode` duplicates when one canonical path is selected.
- Do not pin a vendor model unless the project explicitly requires it.
- Do not enable remote Skills URLs, MCP, browser, shell, or automation.

## Platform-specific audit requirements

- The audit identified extremely short Kilo agents and Skills, especially in area 05. Replace them with complete professional definitions.
- Verify name-to-directory matching for every Skill.
- Use ordered permission rules with deny-by-default behavior.
...
A platform-level coverage or native-capability record may be created under `kilo-code/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(kilo-code): migrate and harden complete cybersecurity specialization
```
