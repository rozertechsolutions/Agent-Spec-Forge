...
## Platform Implementation and Remediation Prompt: Claude Code
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Claude Code**.
...
- CLAUDE.md and current repository-scoped instruction mechanisms
- Custom subagents
- Agent Skills
- Slash commands or other current reusable prompt mechanisms
- Lifecycle hooks
- Project settings, permissions, and MCP configuration
...
- Use only current repository-scoped Claude Code formats and precedence.
- Create a small coordinator plus focused specialist subagents and an independent reviewer where justified.
- Use Skills for reusable area workflows and schemas.
- Use prompt-based or declarative safety controls when possible. Any command hook created must be minimal, reviewable, non-destructive, and must not be executed during this task.
- Omit MCP unless a real optional integration contract exists; never connect it.

## Explicit platform exclusions and corrections

- Do not create Claude web/desktop Project files.
- Do not mix legacy Claude Code formats with current formats.
- Do not create executable filler hooks or active MCP servers.
- Do not broaden permissions beyond the minimum necessary for static repository work.

## Platform-specific audit requirements

- Audit every existing subagent, Skill, setting, command, and hook for current schema and path accuracy.
- Expand any brief agent or Skill that lacks the mandatory quality contract.
- Ensure permissions prohibit shell execution, external access, installs, builds, tests, deployments, scans, and Git remote operations.
...
```text
claude-code/cybersecurity/<area>/
```
...
A platform-level coverage or native-capability record may be created under `claude-code/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(claude-code): harden complete cybersecurity specialization
```
