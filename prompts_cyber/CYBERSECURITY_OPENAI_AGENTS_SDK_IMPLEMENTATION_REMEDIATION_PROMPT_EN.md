...
## Platform Implementation and Remediation Prompt: OpenAI Agents SDK
...
```text
openai-agents-sdk/cybersecurity/
```
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **OpenAI Agents SDK**.
...
- Human-in-the-loop approval interruptions
- Run state and resumable approval design
- MCP interfaces and tools kept disabled
- Tracing explicitly disabled by default for this static safe package
...
- Replace shallow role-specification-only areas with real, consistently wired SDK agents.
- For each area, create a coordinator, focused specialists, and an independent assurance agent using actual SDK constructs.
- Use typed Pydantic outputs for assessments, findings, decisions, evidence requests, approvals, and assurance results.
- Use explicit handoffs or agents-as-tools with no circular delegation.
- Implement input and output guardrails for authorization, scope, secrets, active-testing requests, unsupported completion claims, and human-only decisions.
- Use HITL approval definitions for any future sensitive tool call, while keeping all tools empty or disabled in this repository.
- Disable tracing in code by default to avoid unintended data export.
- Add the missing defensive-operations area.

## Explicit platform exclusions and corrections

- Do not run the SDK, install dependencies, call a model, create API clients, use API keys, start a server, create a UI, or connect MCP.
- Do not leave an area as only a list of strings or unconnected Agent objects.
- Do not use tools that perform shell, computer, web, file-search, code-interpreter, hosted MCP, or external actions.
- Do not create a deployment, installer, Dockerfile, package installation command, or authentication flow.

## Platform-specific audit requirements

- The audit found inconsistent wiring and no area 05. Correct all eight areas.
- Create static wiring tests or inspection fixtures only if they are pure source artifacts; do not execute them.
- Ensure the import surface and module boundaries are consistent across areas.
- Document exact SDK version assumptions without pinning an unverified future version.
...
```text
openai-agents-sdk/cybersecurity/<area>/
```
...
A platform-level coverage or native-capability record may be created under `openai-agents-sdk/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(openai-agents-sdk): complete production-grade cybersecurity agent wiring
```
