...
## Platform Implementation and Remediation Prompt: Windsurf / Devin Desktop
...
Create a professional, adaptable, organization-neutral Cybersecurity baseline that is functionally specialized and implemented exclusively through the current native capabilities of **Windsurf / Devin Desktop**.
...
- AGENTS.md
- Workspace Skills
- Cascade Workflows where the selected current surface supports them
- Cascade Hooks
- MCP
- Devin Local granular permissions and subagent-assisted context
...
- Windsurf Docs — Rules and Memories
- Windsurf Docs — Skills
- Windsurf Docs — AGENTS.md
- Windsurf Docs — Workflows
- Windsurf Docs — Cascade Hooks
- Windsurf Docs — MCP
- Windsurf Docs — Devin Local Agent differences and permissions
...
- First identify the current selected product surface: Cascade or Devin Local.
- Use shared current mechanisms—AGENTS.md, Rules, and Skills—where possible.
- If targeting Devin Local, migrate workflow content into Skills because current documentation states Workflows are unavailable there.
- If retaining Cascade Workflows, state that they are manual slash-command workflows and not available in Devin Local.
- Use hooks and permissions only through the exact selected surface.
- Add the missing defensive-operations area.

## Explicit platform exclusions and corrections

- Do not mix Cascade-only and Devin-Local-only files without explicit surface documentation.
- Do not create global memories or rely on auto-generated memories as version-controlled configuration.
- Do not connect MCP, enable app deploys, or create cloud tasks.
- Do not add executable hook scripts unless strictly necessary, safe, and fully reviewed; never execute them during implementation.

## Platform-specific audit requirements

- The audited package lacks area 05; create it.
- Review current product renaming and migration behavior.
- Use Skills instead of Workflows for the broadest current compatibility.
- Ensure permissions deny commands, external HTTP, MCP, and unrelated writes.
...
A platform-level coverage or native-capability record may be created under `windsurf/cybersecurity/` only when it provides substantive traceability and does not simulate a platform feature.
...
```text
fix(windsurf): complete and harden cybersecurity specialization
```
