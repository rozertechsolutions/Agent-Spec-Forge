...
No connector is preselected or enabled. Use an explicit allowlist per task. Grant the smallest data scope, require human confirmation before write actions, prohibit credential copying into prompts, and record any data leaving the workspace. Disconnect access when no longer needed.
