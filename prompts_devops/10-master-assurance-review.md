# Master Implementation Prompt — DevOps and Cloud — 10 Assurance and Independent Review

You are implementing exactly one validated section of the open-source **AI Specialized Departments** repository.

Your task is to implement **Assurance and Independent Review** sequentially across the supported platform packages. Work autonomously within the rules below. Do not ask for confirmation unless repository safety makes progress impossible. Do not begin another DevOps and Cloud section.
...
## Section contract
    ...
    **Section:** `10-assurance-review`  
    **Mission:** Provide independent, evidence-based assurance that all DevOps and Cloud outputs are complete, native, safe, coherent and professionally usable.

    ### Roles and non-overlapping ownership

    | Role | Exclusive ownership |
|---|---|
| `DevOps and Cloud Assurance Reviewer` | Independently reviews architecture, delivery, infrastructure, containers, reliability, resilience, performance, DevSecOps, FinOps and evidence. It may block completion but must not silently rewrite or self-review. |
...
- IaC, pipeline, container and Kubernetes review
- Operational readiness, SLO, observability and incident-readiness review
- Resilience, recovery, performance and capacity review
- DevSecOps and software supply-chain review
- FinOps and sustainability evidence review
- Traceability, ownership, overlap and contradiction detection
- Completion gates, findings, waivers and re-review

    ### Explicit exclusions

    - Creating the primary implementation being reviewed
...
- Claiming runtime validation from static inspection
    ...
    - AWS, Azure and Google Cloud Well-Architected frameworks
- DORA, Google SRE, NIST SSDF/SP 800-204D, CNCF platform guidance and FinOps Framework
- Static review of all technology-specific configuration generated by prior sections
...
- Verify that the content is specific to `Assurance and Independent Review`, not generic DevOps filler.
...
3. Commit with:
   **`feat(devops-cloud): implement 10 assurance review`**
