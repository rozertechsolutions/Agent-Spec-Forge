# Master Implementation Prompt — DevOps and Cloud — 08 DevSecOps

You are implementing exactly one validated section of the open-source **AI Specialized Departments** repository.

Your task is to implement **DevSecOps** sequentially across the supported platform packages. Work autonomously within the rules below. Do not ask for confirmation unless repository safety makes progress impossible. Do not begin another DevOps and Cloud section.

## Repository and execution contract

- Repository: **AI Specialized Departments**.
- Working branch: **`feature/devops-cloud`**.
- The branch `feature/devops-cloud` must already exist from Section 01. Fetch it if necessary, switch to it and stop if it is absent or not based on `main`.
...
## Section contract
    ...
    **Mission:** Integrate security into delivery, infrastructure, cloud and container workflows while leaving enterprise security governance, offensive security, SOC and security incident response to Cybersecurity.

    ### Roles and non-overlapping ownership

    | Role | Exclusive ownership |
|---|---|
| `DevSecOps Engineer` | Owns secure delivery controls, pipeline security requirements, security gate placement, findings routing and developer-facing remediation workflows. |
| `Cloud Security Controls Engineer` | Owns technical cloud IAM, secrets controls, policy as code, cloud configuration hardening and least-privilege patterns inside DevOps workflows. |
| `Software Supply Chain Security Engineer` | Owns dependency and artifact integrity, SBOM, provenance, signing, verification, build isolation and software supply-chain controls. |
    ...
    - Pipeline and runner security
- Least privilege, workload identity and secrets management
- SAST, DAST, SCA and secret scanning integration design
- IaC, container and Kubernetes scanning
- Policy as code using OPA, Gatekeeper, Kyverno or equivalent where justified
- SBOM, artifact signing, provenance and verification
- SLSA-aligned supply-chain controls
- Security gates, exception handling and remediation ownership
- NIST SSDF and SP 800-204D alignment

    ### Explicit exclusions

    - Pentesting, threat intelligence, SOC/SIEM operations and digital forensics
- Enterprise GRC ownership
- Executing scanners, signing artifacts or accessing secret stores
- Automatic blocking or production enforcement without human approval

    ### Technology knowledge to cover without creating one agent per product

    - SAST, DAST, SCA and secret-scanning categories
- IaC, container and Kubernetes security scanners
- OPA, Gatekeeper and Kyverno
- SBOM standards and tooling patterns
- Sigstore/Cosign-style signing and verification
- SLSA provenance and build integrity
- Cloud IAM and secrets managers
...
- Verify that the content is specific to `DevSecOps`, not generic DevOps filler.
...
3. Commit with:
   **`feat(devops-cloud): implement 08 devsecops`**
