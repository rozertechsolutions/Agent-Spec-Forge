# Master Implementation Prompt — DevOps and Cloud — 02 Cloud Foundation and Infrastructure

You are implementing exactly one validated section of the open-source **AI Specialized Departments** repository.

Your task is to implement **Cloud Foundation and Infrastructure** sequentially across the supported platform packages. Work autonomously within the rules below. Do not ask for confirmation unless repository safety makes progress impossible. Do not begin another DevOps and Cloud section.
...
## Section contract
    ...
    **Section:** `02-cloud-foundation-infrastructure`  
    **Mission:** Define safe, reproducible and provider-aware cloud foundations and infrastructure configuration while remaining neutral until requirements justify a provider or tool.
...
| `Cloud Network Engineer` | Owns VPC/VNet design, subnets, routing, DNS, load balancing, ingress/egress, connectivity and network segmentation at the cloud/platform layer. |
| `Cloud Runtime and Managed Services Engineer` | Owns infrastructure configuration for compute, serverless, storage, managed databases, caches, queues, streams and managed runtime services without owning application logic or data modelling. |
    ...
    - Organizations, accounts, subscriptions, projects and landing zones
- Environment isolation and promotion boundaries
- Infrastructure as code, state, modules, drift and configuration management
- Networks, DNS, load balancing, connectivity and service endpoints
- Compute, serverless, storage, managed data services, caches and messaging infrastructure
- Resource lifecycle, tagging, ownership and decommissioning
- Terraform, OpenTofu, Pulumi, CloudFormation, Bicep and Ansible knowledge where justified

    ### Explicit exclusions

    - Application business logic, database schema design and analytics pipelines
- Actual provisioning, plan/apply execution or cloud authentication
- Enterprise-wide identity governance beyond technical cloud controls
    ...
    - AWS, Azure and Google Cloud foundations and managed services
- Terraform and OpenTofu
- Pulumi
- AWS CloudFormation
- Azure Bicep
- Ansible and declarative configuration management
...
- Verify that the content is specific to `Cloud Foundation and Infrastructure`, not generic DevOps filler.
...
3. Commit with:
   **`feat(devops-cloud): implement 02 cloud foundation infrastructure`**
