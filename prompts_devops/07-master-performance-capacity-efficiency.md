# Master Implementation Prompt — DevOps and Cloud — 07 Performance, Capacity and Efficiency

You are implementing exactly one validated section of the open-source **AI Specialized Departments** repository.

Your task is to implement **Performance, Capacity and Efficiency** sequentially across the supported platform packages. Work autonomously within the rules below. Do not ask for confirmation unless repository safety makes progress impossible. Do not begin another DevOps and Cloud section.
...
## Section contract
    ...
    **Section:** `07-performance-capacity-efficiency`  
    **Mission:** Define measurable performance and capacity plans that support reliability and cost decisions without running benchmarks or modifying real infrastructure.
...
| `Performance and Capacity Engineer` | Owns performance requirements, workload models, load/stress/soak test design, bottleneck analysis, capacity planning, autoscaling strategy and technical resource efficiency. |
    ...
    - Latency, throughput, concurrency, saturation and performance budgets
- Load, stress, spike, endurance and scalability test design
- Capacity models, demand forecasts and headroom
- Horizontal and vertical scaling
- Autoscaling signals, limits and stabilization
- Resource requests, limits and efficiency
- Bottleneck analysis and performance regression criteria

    ### Explicit exclusions

    - Executing load tests or benchmarks
- Pure financial optimization owned by FinOps
- Application-level algorithm optimization owned by development teams unless it affects platform guidance

    ### Technology knowledge to cover without creating one agent per product

    - Load-testing and benchmarking tool patterns without execution
- Kubernetes autoscaling and resource management
- Cloud autoscaling services
- Observability-derived capacity signals
- Queueing, caching and graceful-degradation patterns
...
- Verify that the content is specific to `Performance, Capacity and Efficiency`, not generic DevOps filler.
...
3. Commit with:
   **`feat(devops-cloud): implement 07 performance capacity efficiency`**
