# Master Implementation Prompt — DevOps and Cloud — 06 Resilience and Disaster Recovery

You are implementing exactly one validated section of the open-source **AI Specialized Departments** repository.

Your task is to implement **Resilience and Disaster Recovery** sequentially across the supported platform packages. Work autonomously within the rules below. Do not ask for confirmation unless repository safety makes progress impossible. Do not begin another DevOps and Cloud section.
...
## Section contract
    ...
    **Section:** `06-resilience-disaster-recovery`  
    **Mission:** Design recoverable systems and verifiable continuity mechanisms without performing destructive tests, failovers or production recovery actions.
...
- Backup, restore, retention and data-integrity strategy
- RTO, RPO and business-service dependency mapping
- Disaster recovery patterns and runbooks
- Failover, failback and regional recovery
- Chaos engineering and game-day design under strict human control
- Recovery test evidence and unresolved-gap tracking

    ### Explicit exclusions

    - Executing backup restoration, chaos experiments or failover
...
- Database and storage recovery patterns
- Chaos engineering tools as design references only
...
- Verify that the content is specific to `Resilience and Disaster Recovery`, not generic DevOps filler.
...
3. Commit with:
   **`feat(devops-cloud): implement 06 resilience disaster recovery`**
