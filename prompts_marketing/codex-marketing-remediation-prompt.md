# AI Specialized Departments — OpenAI Codex Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `codex/` in the existing
...
1. Inventory every file under `codex/marketing/`.
...
Engineering lead and independent reviewer have workspace-write, generic workflow duplication, and unsafe approval-state semantics.

## Deletion allowlist

- `codex/marketing/AUDIT.md`
- `codex/marketing/marketing-professional/workflows/`
- `codex/marketing/marketing-engineering/workflows/`

No other file or directory may be deleted.

## Additions and migrations

- Merge unique workflow instructions into the corresponding native
  `.agents/skills/<skill>/SKILL.md` packages before deleting the workflow directories.
- Add Skill-local references only when required to preserve unique workflow details.

## Required modifications

- In `marketing-engineering/.codex/agents/` set:
  - `marketing-engineering-lead.toml`: `sandbox_mode = "read-only"`;
  - `marketing-engineering-security-qa-reviewer.toml`:
    `sandbox_mode = "read-only"`.
- Keep workspace writing only for the seven implementation specialists whose contracts
  require bounded file changes.
- Keep network access disabled by default for every agent.
- Keep all Professional agents read-only.
- Ensure no agent uses unrestricted sandboxing or automatic command approval.
- Update both `.codex/config.toml` files, `AGENTS.md`, agent instructions, Skills,
  catalogs, README, installation, governance, handoff, and templates.

## Platform-specific acceptance criteria

- Both `codex/marketing/marketing-professional/` and
  `codex/marketing/marketing-engineering/` remain present and independently usable.
