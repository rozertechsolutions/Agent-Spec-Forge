# AI Specialized Departments — Kiro Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `kiro/` in the existing
...
1. Inventory every file under `kiro/marketing/`.
...
Reusable procedures are incorrectly kept in persistent steering, generic specs simulate active work, workflows are duplicated, governance is duplicated, and approval states are unsafe.

## Deletion allowlist

- `kiro/marketing/AUDIT.md`
- `kiro/marketing/marketing-professional/reference/GOVERNANCE.md`
- `kiro/marketing/marketing-engineering/reference/GOVERNANCE.md`
- `kiro/marketing/marketing-professional/.kiro/steering/procedures/`
- `kiro/marketing/marketing-engineering/.kiro/steering/procedures/`
- `kiro/marketing/marketing-professional/.kiro/specs/`
- `kiro/marketing/marketing-engineering/.kiro/specs/`
- `kiro/marketing/marketing-professional/workflows/`
- `kiro/marketing/marketing-engineering/workflows/`
...
- Create:
  - `marketing-professional/.kiro/skills/`
  - `marketing-engineering/.kiro/skills/`
- For every reusable procedure currently under `.kiro/steering/procedures/`, create a
  native `.kiro/skills/<skill-name>/SKILL.md` and preserve all unique procedure detail.
- Merge unique detail from generic pre-created specs and top-level workflows into the
  matching Skills before deleting the authorized directories.

## Required modifications

- Keep `.kiro/steering/` only for concise persistent scope, governance, ownership,
  safety, and role-selection guidance.
- Keep `.kiro/steering/ownership-and-approval.md` as the sole governance source in each
  branch and update all references.
- Do not ship pre-created generic specs that pretend a real project task is active.
  Documentation must explain that users create a fresh Kiro spec for a concrete feature
  or remediation task.
- Do not add active hooks by default because hooks may execute prompts or shell commands.
- Update steering, Skills, catalogs, README, installation, handoff, templates, source
  notes, and approval states.

## Platform-specific acceptance criteria

- Both `kiro/marketing/marketing-professional/` and
  `kiro/marketing/marketing-engineering/` remain present and independently usable.
