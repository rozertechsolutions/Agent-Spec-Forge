# AI Specialized Departments — OpenAI Agents SDK Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `openai-agents-sdk/` in the existing
`AI-Specialized-Departments` repository.

Required branches:

```text
openai-agents-sdk/marketing/
...
1. Inventory every file under `openai-agents-sdk/marketing/`.
...
Agents are mostly tool-less, orchestration is prompt text instead of staged execution, HITL approval is not implemented correctly, tests are minimal, and model-generated human approval remains possible.

## Deletion allowlist

- `openai-agents-sdk/marketing/AUDIT.md`
- `openai-agents-sdk/marketing/marketing-professional/workflows/`
- `openai-agents-sdk/marketing/marketing-engineering/workflows/`

No other file or directory may be deleted.

## Additions and migrations

For each branch, add these files under the existing package:

Professional package `src/marketing_professional_agents/`:
- `approval.py`
- `tools.py`
- `workflows.py`

Engineering package `src/marketing_engineering_agents/`:
- `approval.py`
- `tools.py`
- `workflows.py`

For each branch add:
- `docs/workflows/` containing the moved human-readable workflow documents;
- `tests/test_approval.py`;
- `tests/test_tools.py`;
- `tests/test_orchestration.py`;
- `tests/test_guardrails.py`.

Move the Markdown workflow documents into `docs/workflows/` before deleting the original
top-level `workflows/` directories.

## Required modifications

- Remove `HUMAN_APPROVED` from both `WorkState` enums and every agent output model.
- Implement a separate externally supplied approval record in `approval.py`; it must
  never be model-generated.
- Implement safe bounded workspace tools in `tools.py` only where the role requires
  them:
  - explicit allowed workspace root;
  - path normalization and traversal rejection;
  - sensitive-file denial;
  - no network, deployment, publication, sending, spending, credential, destructive,
    or production tools;
  - every write-capable tool uses SDK `needs_approval` HITL interruptions.
- Keep Professional roles read-only unless a clearly justified draft-file tool is
  approval-gated.
- Keep Engineering lead and security/QA reviewer tool-less or read-only.
- Implement actual staged orchestration in `workflows.py` and/or
  `orchestration.py`: run the required specialist stages in order, preserve stage
  outputs, enforce stop gates, invoke a distinct reviewer, and end at readiness for
  human approval.
- Do not merely append workflow stage names to the user prompt.
- Use handoffs or agents-as-tools according to current SDK guidance and keep ownership
  explicit.
- Strengthen input, output, and tool guardrails with structural validation for state,
  evidence, reviewer identity, forbidden external-action flags, and unauthorized tools.
- Ensure RunState interruptions can be serialized, approved/rejected externally, and
  resumed without the model inventing approval.
- Expand tests to cover state restrictions, traversal, sensitive paths, HITL
  interruption/rejection, stage order, stop gates, creator/reviewer separation,
  guardrail tripwires, disabled external actions, and invalid workflow names.
- Update `__init__.py`, agents, models, context, guardrails, orchestration,
  `pyproject.toml`, README, USAGE, INSTALLATION, docs, catalogs, schemas, templates,
  source notes, and exports.
- Use a current justified compatible dependency range; do not pin an obsolete minimum
  without an upper compatibility bound.
- Do not run the tests in this task; validate code structure and imports statically.

## Platform-specific acceptance criteria

- Both `openai-agents-sdk/marketing/marketing-professional/` and
  `openai-agents-sdk/marketing/marketing-engineering/` remain present and independently usable.
