# AI Specialized Departments — Warp / Oz Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `warp/` in the existing
...
1. Inventory every file under `warp/marketing/`.
...
- https://docs.warp.dev/platform/skills-as-agents/
...
Top-level workflows duplicate Skill procedures, independent reviewer execution is insufficiently documented, MCP guidance can be mistaken for active config, and approval states are unsafe.

## Deletion allowlist

- `warp/marketing/AUDIT.md`
- `warp/marketing/marketing-professional/workflows/`
- `warp/marketing/marketing-engineering/workflows/`

No other file or directory may be deleted.

## Additions and migrations

- Merge unique workflow content into `.warp/skills/` before deleting top-level workflow
  directories.
- Add Skill-local references where necessary.

## Required modifications

- Keep `AGENTS.md` for persistent project guidance and `.warp/skills/` for reusable
  procedures.
- Document how independent reviewers should run as separate Skills-as-Agents and how a
  parent agent may use a reviewer child, without claiming file-based per-agent
  permissions that Warp does not enforce through these files.
- Clarify that `MCP.md` is a human safety/configuration guide, not an enabled MCP server
  configuration.
- Keep integrations, schedules, cloud agents, connectors, and MCP disabled and
  user-configured by default.
- Do not create an Oz environment, schedule, API configuration, token, or active
  integration.
- Update AGENTS, Skills, MCP guidance, catalogs, README, installation, governance,
  handoff, templates, source notes, and approval states.

## Platform-specific acceptance criteria

- Both `warp/marketing/marketing-professional/` and
  `warp/marketing/marketing-engineering/` remain present and independently usable.
