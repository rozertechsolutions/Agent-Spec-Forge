# AI Specialized Departments — GitHub Copilot Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `github-copilot/` in the existing
...
1. Inventory every file under `github-copilot/marketing/`.
...
Engineering lead and reviewer have edit access, workflows duplicate Skills, validation claims are too strong, and approval states are unsafe.

## Deletion allowlist

- `github-copilot/marketing/AUDIT.md`
- `github-copilot/marketing/marketing-professional/workflows/`
- `github-copilot/marketing/marketing-engineering/workflows/`

No other file or directory may be deleted.

## Additions and migrations

- Merge unique workflow content into `.github/skills/` before deleting the top-level
  workflow directories.
- Add Skill-local reference documents only where needed.

## Required modifications

- In `marketing-engineering/.github/agents/`, remove `edit` from:
  - `marketing-engineering-lead.agent.md`;
  - `marketing-engineering-security-qa-reviewer.agent.md`.
- Keep editing tools only for the seven implementation specialists.
- Use only tool identifiers supported by the targeted Copilot surface; do not invent a
  terminal or test tool.
- When executable validation is unavailable, require implementers to create tests and a
  validation plan and reviewers to inspect externally supplied evidence.
- State which Copilot surfaces the configuration targets and do not claim identical
  behavior across CLI, IDE, cloud coding agent, and GitHub.com.
- Update instructions, path-specific instructions, agents, Skills, catalogs, README,
  installation, governance, handoff, templates, and approval states.

## Platform-specific acceptance criteria

- Both `github-copilot/marketing/marketing-professional/` and
  `github-copilot/marketing/marketing-engineering/` remain present and independently usable.
