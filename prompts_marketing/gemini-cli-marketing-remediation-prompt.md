# AI Specialized Departments — Gemini CLI Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `gemini-cli/` in the existing
`AI-Specialized-Departments` repository.

Required branches:

```text
gemini-cli/marketing/
...
1. Inventory every file under `gemini-cli/marketing/`.
...
Engineering implementers lack bounded write capability, generic workflows duplicate Skills, and approval states are unsafe.

## Deletion allowlist

- `gemini-cli/marketing/AUDIT.md`
- `gemini-cli/marketing/marketing-professional/workflows/`
- `gemini-cli/marketing/marketing-engineering/workflows/`

No other file or directory may be deleted.

## Additions and migrations

- Merge unique workflow content into `.gemini/skills/` before deleting top-level
  workflow directories.
- Add Skill-local references only if needed to retain unique procedure detail.

## Required modifications

- Keep Professional agents read/search-only.
- In `marketing-engineering/.gemini/agents/`, keep the Engineering lead and
  security/QA reviewer read/search-only.
- Give the seven implementation specialists only the current documented file writing
  or replacement tools needed for bounded workspace changes.
- Do not enable shell execution by default. If a role's contract mentions validation,
  require it to prepare tests and consume externally supplied execution evidence rather
  than claim a test ran.
- Do not add repository policies that official documentation still marks experimental
  or unsupported for stable workspace enforcement.
- Do not add executable hooks by default; document them only as optional user-managed
  extensions if useful.
- Update `GEMINI.md`, all agent tool lists, Skills, catalogs, README, installation,
  governance, handoff, templates, source notes, and approval states.

## Platform-specific acceptance criteria

- Both `gemini-cli/marketing/marketing-professional/` and
  `gemini-cli/marketing/marketing-engineering/` remain present and independently usable.
