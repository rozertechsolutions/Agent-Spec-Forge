# AI Specialized Departments — Cline Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `cline/` in the existing
...
1. Inventory every file under `cline/marketing/`.
...
Duplicated governance, generic workflow duplication, procedural reviewer separation, and unsafe approval-state semantics.

## Deletion allowlist

- `cline/marketing/AUDIT.md`
- `cline/marketing/marketing-professional/reference/GOVERNANCE.md`
- `cline/marketing/marketing-engineering/reference/GOVERNANCE.md`
- `cline/marketing/marketing-professional/workflows/`
- `cline/marketing/marketing-engineering/workflows/`

No other file or directory may be deleted.

## Additions and migrations

- Merge unique workflow content into the corresponding native
  `.cline/skills/<skill>/SKILL.md` packages before deleting the top-level workflow
  directories.
- Add Skill-local reference files only when necessary to preserve unique workflow
  details.

## Required modifications

- Keep `.cline/rules/ownership-and-approval.md` as the authoritative ownership and
  approval rule in each branch and update all references.
- Keep `.cline/rules/marketing-governance.md` and `.cline/skills/`.
- State that Cline Plan mode is read-only and Act mode performs changes only after user
  approval.
- Do not invent repository agent permission files or claim isolated reviewers unless
  current official documentation provides a stable project-native mechanism.
- Require a separate task/session or human reviewer for independent review when native
  isolation is unavailable.
- Update rules, Skills, catalogs, README, installation, handoff, templates, and states.

## Platform-specific acceptance criteria

- Both `cline/marketing/marketing-professional/` and
  `cline/marketing/marketing-engineering/` remain present and independently usable.
