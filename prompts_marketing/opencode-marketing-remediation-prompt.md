# AI Specialized Departments — OpenCode Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `opencode/` in the existing
...
1. Inventory every file under `opencode/marketing/`.
...
Engineering lead/reviewer may edit, workflows duplicate Skills, permissions need least privilege, and approval states are unsafe.

## Deletion allowlist

- `opencode/marketing/AUDIT.md`
- `opencode/marketing/marketing-professional/workflows/`
- `opencode/marketing/marketing-engineering/workflows/`

No other file or directory may be deleted.

## Additions and migrations

- Merge unique workflow content into `.opencode/skills/` before deleting top-level
  workflow directories.
- Add Skill-local references only where necessary.

## Required modifications

- Use the current `permission` model, not deprecated tool configuration.
- In Engineering set both `edit: deny` and `bash: deny` for:
  - `marketing-engineering-lead`;
  - `marketing-engineering-security-qa-reviewer`.
- Keep implementation specialists at `edit: ask` or narrower path-based rules.
- Keep Bash denied by default in the repository package. Validation contracts must use
  externally supplied evidence rather than claim commands ran.
- Deny external directories and sensitive files.
- Update `opencode.json`, agent frontmatter/configuration, Skills, catalogs, README,
  installation, governance, handoff, templates, source notes, and approval states.

## Platform-specific acceptance criteria

- Both `opencode/marketing/marketing-professional/` and
  `opencode/marketing/marketing-engineering/` remain present and independently usable.
