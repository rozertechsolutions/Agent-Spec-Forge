# AI Specialized Departments — Claude Code Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `claude-code/` in the existing
...
1. Inventory every file under `claude-code/marketing/`.
...
Engineering implementers are read-only, generic workflows duplicate Skills, and approval states allow model-generated human approval.

## Deletion allowlist

- `claude-code/marketing/AUDIT.md`
- `claude-code/marketing/marketing-professional/workflows/`
- `claude-code/marketing/marketing-engineering/workflows/`

No other file or directory may be deleted.

## Additions and migrations

- Merge every unique workflow step into the corresponding native
  `.claude/skills/<skill>/SKILL.md` before deleting the top-level workflow directories.
- Add supporting Skill reference files only when required to preserve unique workflow
  detail; keep them inside the relevant Skill directory.

## Required modifications

- In `marketing-engineering/.claude/agents/`, keep
  `marketing-engineering-lead.md` and
  `marketing-engineering-security-qa-reviewer.md` restricted to read, search, and
  research tools.
- Give only these implementation agents the current officially supported bounded file
  editing tools:
  `measurement-tracking-engineer`, `technical-seo-engineer`,
  `web-landing-page-engineer`, `martech-integration-engineer`,
  `marketing-data-analytics-engineer`, `privacy-consent-governance-engineer`, and
  `experimentation-cro-engineer`.
- Do not grant edit/write tools to the lead or reviewer.
- Do not grant Bash by default. If official current behavior requires it for a declared
  validation step, keep it approval-gated and update the contract so no agent claims a
  command ran without supplied evidence.
- Keep Professional agents read-only.
- Update `CLAUDE.md`, all agent tool lists, Skills, catalogs, README, installation,
  governance, handoff, and templates to match the corrected permissions and migrated
  workflows.
- Do not add active executable hooks merely to fill a capability category.

## Platform-specific acceptance criteria

- Both `claude-code/marketing/marketing-professional/` and
  `claude-code/marketing/marketing-engineering/` remain present and independently usable.
