# AI Specialized Departments — Qwen Code Marketing Remediation Prompt

## Mission

Correct only the Marketing specialization for `qwen-code/` in the existing
...
1. Inventory every file under `qwen-code/marketing/`.
...
All Engineering agents use plan mode, preventing implementation, workflows duplicate Skills, and approval states are unsafe.

## Deletion allowlist

- `qwen-code/marketing/AUDIT.md`
- `qwen-code/marketing/marketing-professional/workflows/`
- `qwen-code/marketing/marketing-engineering/workflows/`

No other file or directory may be deleted.

## Additions and migrations

- Merge unique workflow content into `.qwen/skills/` before deleting top-level workflow
  directories.
- Add Skill-local references only where needed.

## Required modifications

- Keep all Professional agents in `approvalMode: plan`.
- In Engineering keep these agents in `approvalMode: plan` and without write tools:
  - `marketing-engineering-lead`;
  - `marketing-engineering-security-qa-reviewer`.
- Change the seven Engineering implementation specialists to
  `approvalMode: default` or the current equivalent that requires interactive approval
  for file edits.
- Give implementation specialists only documented read, search, write, and replace
  tools required for bounded workspace changes.
- Do not use `auto-edit`, `yolo`, or any privileged mode as a repository default.
- Do not enable shell by default. Require externally supplied validation evidence.
- Update `QWEN.md`, agents, Skills, catalogs, README, installation, governance, handoff,
  templates, source notes, and approval states.

## Platform-specific acceptance criteria

- Both `qwen-code/marketing/marketing-professional/` and
  `qwen-code/marketing/marketing-engineering/` remain present and independently usable.
